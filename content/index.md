...
**Stats**: 153 papers | 155 wiki pages | 26 categories (17 paper stubs — joonanlab pull queue + dev↔plasticity reuse cluster + cancer↔development cluster 2026-05-10) | 14 overview pages
...
- [[genomic-dl/qin-2026-generator-v2-reconciling-coarse-tokenization]] — **GENERator-v2** (stub, joonanlab pull): coarse tokenization vs single-nucleotide resolution 긴장 해소를 시도한 DNA LM; regulatory variant scoring benchmark 만들 때 token boundary artifact 평가 reference (bioRxiv 2026, prof Joon An framing)
...
- [[single-cell-dl/zhang-2026-single-cell-splicing-analysis-with]] — **ISSAC** (stub, joonanlab pull): single-cell splicing analysis tool로 cell type-specific + cell state-dependent **sQTL** discovery; neurological disorder GWAS overlap; brain atlas의 expression 외 isoform/splicing layer 추가의 method reference (preprint 2026, prof Joon An framing — CPEB4 microexon 등 splice-NDD hypothesis 연결)
...
- [[neuroscience/emani-2024-single-cell-genomics-and-regulatory]] — **PsychENCODE2 flagship resource** (Gerstein + Geschwind + Sestan + Allen + 컨소시엄). 388 adult PFC × 2.8M nuclei × 28 BICCN cell types snRNA+snATAC+multiome+WGS → 1.4M scQTL + 550K scCRE + cell-type GRN + cell-cell communication network → **LNCTP integrative model** (genotype→cell-type-expression imputation + 223 prioritized SCZ/BPD/ASD/AD risk gene-cell-type triplets + in silico perturbation) (Science 2024)
- [[neuroscience/gordon-2026-developmental-convergence-and-divergence]] — **Pasca + Geschwind 통합 작품**. 70 patient-derived hiPSC lines (8 ASD mutations + 11 idiopathic + 25 controls) cortical organoid 4-time-point RNA-seq → **early divergence → late convergence** + **M5 chromatin/TF regulator complex (SMARCA4, SMARCB1, EP300, TP53...) = upstream causal driver** (regulates 261 SFARI genes); IP-MS + CRISPRi 검증; **idiopathic ASD에는 convergent signal 없음** (Nature 2026)
- [[neuroscience/stirtz-2026-a-genetic-screen-reveals-dosage]] — **Stirtz dosage screen** (stub, joonanlab pull): high-confidence ASD risk gene을 Drosophila dosage screen으로 평가, **domino** (chromatin TF)를 synaptic+behavioral phenotype regulator로 prioritize; 데기디바 검증 파이프라인의 compact model (bioRxiv 2026, prof framing)
- [[neuroscience/udeshi-2026-shank3-anchored-reverse-phenotyping-identifies]] — **Udeshi reverse phenotyping** (stub, joonanlab pull): SHANK3 carrier anchor로 ASD 안의 cognitive-motor subgroup 정의; gene-first → phenotype subgroup template (CPEB4/SCN2A/CHD8 적용 가능) (preprint 2026, prof framing)
- [[neuroscience/zhang-2024-shank3-mutations-associated-with-autism]] — **Zhang SHANK3 spine dynamics** (stub, joonanlab pull): SHANK3 mutation class별 dendritic spine dynamics (turnover/maturation, static count 아님) in developing mouse brain; iPSC/organoid synapse readout 설계의 dynamics-vs-count reference (Neuroscience 2024, prof framing)
- [[neuroscience/berndt-2026-sleep-as-a-window-into]] — **Berndt 22q11.2 EEG model** (stub, joonanlab pull): 22q11.2 deletion syndrome sleep EEG를 thalamocortical generative model로 NMDA hypofunction에 연결; 데기디바 cellular perturbation → human intermediate phenotype (EEG) translation framework + cross-disorder NDD reference (medRxiv 2026, prof framing)
- [[neuroscience/bavelier-2010-removing-brakes]] — **Bavelier + Hensch et al. (Rochester + UC Berkeley + Harvard)** symposium (stub, dev↔plasticity reuse cluster): "성인 plasticity는 잠긴 것이지 사라진 것이 아니다" framework. **Brakes** 두 종류 (structural: PNN/myelin; functional: E/I balance) + brake-lifting interventions (chABC, fluoxetine, perceptual learning, action video game). Amblyopia를 animal→human translation model로. 이 cluster의 umbrella review (J Neurosci 2010)
- [[neuroscience/hensch-2005-critical-period-plasticity]] — **Hensch (RIKEN/Harvard)** mechanistic foundation (stub, dev↔plasticity reuse cluster): critical period가 **active, regulated, reversible process**라는 paradigm shift. **PV+ interneuron maturation = opening trigger**, **PNN + Lynx1 + NgR/PirB = closing brakes**. Otx2 cascade로 critical period의 분자 cascade 정립 (Nat Rev Neurosci 2005)
- [[neuroscience/pizzorusso-2002-reactivation-ocular-dominance]] — **Pizzorusso/Maffei (Pisa) + Fawcett (Cambridge)** decisive 실험 (stub, dev↔plasticity reuse cluster): 성체 rat V1에 **chondroitinase ABC** 주입 → PNN 분해 → **monocular deprivation에 반응한 OD shift 회복**. Critical period reopening의 첫 인과적 증거. 77% PNN neurons = PV+ basket cells (Science 2002)
- [[neuroscience/park-poo-2013-neurotrophin-regulation]] — **Park & Mu-ming Poo (UC Berkeley)** comprehensive review (stub, dev↔plasticity reuse cluster): BDNF/TrkB가 발달 wiring (target-derived survival, axon guidance, synapse formation) + 성체 plasticity (LTP maintenance, dendritic spine remodeling) 에 동일 cascade로 작동. **MECP2-CaMKII-Bdnf promoter IV cascade** (Rett의 BDNF 결손 mechanism). **Val66Met SNP** 우울/PTSD 위험. **PTPσ-PNN-TrkB** 상호작용 (chABC reopening의 분자 한 축). 이 cluster의 분자 골격 (Nat Rev Neurosci 2013)
- [[neuroscience/tilot-2015-balancing-proliferation-connectivity-pten]] — **Tilot, Frazier, Eng (Cleveland Clinic)** review (stub, cancer↔development cluster): **PTEN dual-phenotype** (Cowden 암 + ASD with macrocephaly = PHTS) 의 임상 + 분자 통합. 같은 mTOR pathway hyperactivation이 cancer (성체 조직) + macrocephaly+ASD (발달 뉴런) 양쪽의 공통 기반. Pten conditional KO mouse → ASD-like behavior, rapamycin reverse. mTOR inhibitor 임상시험 진행 중. Cancer↔dev thesis의 가장 명확한 임상 single-gene 사례 (Neurotherapeutics 2015)
...
- [[brain-development/zhou-2023-genetics-of-human-brain-development]] — **Foundational review (Song & Ming, UPenn)**. Two-axis frame (evolution + disorder) over the cortical-development timeline; Table 1 of ~30 evolution + ~30 disease genes; mouse-can't-do-this case for hPSC organoids; DISC1 case study as full genotype→treatment pipeline (Nat. Rev. Genet. 2023)
- [[brain-development/lui-2011-development-evolution-human-neocortex]] — **Lui, Hansen, Kriegstein (UCSF)** foundational review (stub, cancer↔development cluster): OSVZ proliferation + oRG (outer radial glia) = 영장류 neocortex 확장의 분자 기반. NE→RG→oRG/IP lineage hierarchy, mouse (lissencephalic) vs primate (gyrencephalic) 차이의 핵심 = OSVZ expansion. Notch/FGF/Wnt regulator 정리. Bhaduri 2020 (oRG-cancer) + Wang 2025 (Tri-IPC) 가 직접 build-on하는 normal-development reference (Cell 2011)
- [[brain-development/bhaduri-2020-outer-radial-glia-like-cancer]] — **Kriegstein lab (UCSF)** GBM single-cell atlas (stub, cancer↔development cluster): IDH-wt GBM에 **oRG-like cancer stem cell population** 발견. 영장류 발달기 oRG의 transcriptome + MST ("jump-and-divide") behavior가 GBM에 reactivate. **PTPRZ1**이 MST + invasive behavior driver. Tumor invasive front에 enrich, 환자 prognosis와 negative correlation. 정상 성체 cortex에 oRG signature absent → cancer-specific therapeutic target. Cancer↔dev thesis의 single-cell 결정적 증거 (Cell Stem Cell 2020)
- [[brain-development/couturier-2020-single-cell-rna-seq-glioblastoma-recapitulates]] — **Petrecca lab (McGill)** GBM hierarchy (stub, cancer↔development cluster): 53,586 GBM cells × 22,637 정상 fetal brain 비교 → **conserved tri-lineage cancer hierarchy** (glial progenitor-like apex → neuronal/astrocytic/oligodendrocytic). Title 자체가 thesis: GBM이 normal neurodevelopmental hierarchy를 recapitulate. RNA velocity로 progenitor가 originator임 증명. IDH-wt GBM에서 처음 tri-lineage hierarchy 입증 (Nat Commun 2020)
...
- [[brain-atlas/consortium-2026-cellnexus-quality-control-annotation-aggregation]] — **cellNexus framework** (stub, joonanlab pull): HCA를 QC + annotation + aggregation + analytical layer 갖춘 reusable resource로 만드는 layered design; raw vs analysis-ready 분리, annotation confidence + sample metadata completeness 강조; brain/organoid atlas curation 운영 standard (bioRxiv 2026, prof framing)
...
- [[organoid/kamzina-2026-the-trem2-r47h-variant-drives]] — **Kamzina TREM2-R47H** (stub, joonanlab pull): AD risk variant가 microglia만이 아니라 forebrain organoid co-culture의 broader neurocellular network를 바꿈; cell-autonomous vs cross-cellular effect 분리 framework (bioRxiv 2026, prof framing — **AD context이므로 ASD/NDD 직접 연결 금지**)
...
- [[overviews/molecular-reuse-dev-plasticity]] — **dev↔adult plasticity 분자 도구 재배치 thesis**: 진화는 adult learning을 위해 새 분자를 발명하지 않고 발달 회로 wiring 도구 (NMDAR/AMPAR silent→active, BDNF/TrkB cascade, CREB/IEG, Reelin, EphA) 를 그대로 가져와 brake (PNN, myelin, Lynx1, E/I balance) 를 추가해 adult plasticity로 재배치. Critical period = open mode → closing trigger = brake 작동. **Reopening evidence (Pizzorusso 2002 chABC)** = brake 풀면 성체에서 critical-period-like plasticity 회복. NDD = cascade or brake 망가져 발달+성체 plasticity 둘 다 결함 (MECP2/Rett, BDNF Val66Met, ASD GABA-excitatory asynchrony). Cluster: Bavelier 2010 + Hensch 2005 + Pizzorusso 2002 + Park & Poo 2013, 영장류 layer는 Wang 2025 Tri-IPC + Keefe 2025 lineage tracing.
- [[overviews/cancer-development-thesis]] — **Cancer = stuck developmental biology thesis**: 같은 분자 도구 (PTEN/mTOR, CHD8/chromatin, oRG/Tri-IPC progenitor signature) 가 발달 시점 잘못되면 NDD, 성체 시점 잘못되면 cancer. **Dual-phenotype gene** (PTEN, CHD8, NF1, TSC, APC, SHH) 가 cancer + NDD risk gene 양쪽 등장. **GBM이 영장류 발달 progenitor (oRG, Tri-IPC) state로 stuck** — Bhaduri 2020 + Couturier 2020 + Wang 2025 Tri-IPC 단일세포 증거. **mTOR inhibitor cross-application** (cancer + ASD 양쪽 임상). [[overviews/molecular-reuse-dev-plasticity]] 와 자매 — 발달 toolkit이 시간 axis로도 (dev↔adult), 조직/시점 axis로도 (정상 dev↔cancer hijack) multi-use. Cluster: Bhaduri 2020 + Couturier 2020 + Wang 2025 + Lui 2011 + Suvà & Tirosh 2019 + Tilot 2015.
- [[overviews/evolution-disease-axis-crossing-patterns]] — **분류 아닌 *읽기 보조***. Zhou 2023 Table 1을 mechanism 정합성 기준으로 다시 읽었을 때 evolution × disease 두 축의 *여러 강도* 교차가 섞여 있다는 관찰. 4가지 패턴 (dosage corridor / tuned-then-vulnerable / pathway-level / process-level) — 단 경계 흐릿 + multi-membership + evidence 강도 가변을 명시. NOTCH2NL (1q21.1 dosage corridor) / ASPM/CDK5RAP2 (tuned-then-vulnerable) / Wnt FZD8↔TCF4 (pathway-level) / neoteny (process-level) 가 anchor 사례. 마지막에 추가 ingest 후보 7편 (Pollen 2023, Sousa 2017, Fiddes/Suzuki 2018 NOTCH2NL, Doan 2016 HARs+ASD, Boyd 2015 HARE5 원본, Gandal 2018, Sullivan-Geschwind 2019). **Soft-framing 원칙 적용 — Mode N 분류 거부**.
...
- [[other/eadie-2026-arrival-of-digital-twins-and]] — **Nature Medicine Comment** (regulatory science): digital twin + in silico trial이 industry tool → regulatory-grade evidence로 전환 시점; 2025-04 FDA monoclonal antibody animal-testing phase-out trigger; 5 regulatory priorities (validation / cross-center / conflict / post-market / pilot); mechanistic vs statistical model trade-off; outer-loop framing for wiki의 perturbation/drug response method 페이퍼들 (Nat. Med. 2026)
- [[other/suva-tirosh-2019-single-cell-rna-sequencing-cancer]] — **Suvà (MGH/Harvard/Broad) + Tirosh (Weizmann)** review (stub, cancer↔development cluster): single-cell RNA-seq cancer 연구 종합. **eITH (expression intra-tumor heterogeneity) 의 3가지 결정 요인** 정리 (genetic + epigenetic/developmental + extrinsic/spatial). 핵심 thesis: **malignant cell heterogeneity가 정상 발달 differentiation hierarchy를 닮음** — IDH-mutant glioma (tri-lineage) + H3-K27M glioma (OPC-stuck) + 멜라노마 (AXL-high vs MITF-high) + head & neck (epithelial/EMT/differentiated) 모두 정상 발달 program이 cancer에 frozen reactivate. Cancer↔dev cluster의 umbrella review (Mol Cell 2019)
